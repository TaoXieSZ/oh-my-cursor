...
4. Write output to `.omr/plans/autopilot-spec.md`.
...
2. Write plan to `.omr/plans/autopilot-impl.md`.
3. Write test spec to `.omr/plans/autopilot-test-spec.md`.
...
2. Write final summary to `.omr/state/autopilot-state.json`.
3. Report to user: what was built, files created, tests passing, any caveats.

## State management

Write to `.omr/state/autopilot-state.json`:
```json
{
