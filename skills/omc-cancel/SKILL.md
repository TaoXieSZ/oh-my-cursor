---
name: omc-cancel
description: Cancel the currently active OMC workflow mode. Use when the user says "cancel", "stop", or "abort".
argument-hint: "[mode to cancel]"
---

# Cancel — Clean Mode Termination

Cleanly cancel the active OMC workflow mode and reset state.
...
1. Identify the active mode(s) from `.omc/state/*-state.json`.
...
State saved to: .omc/state/{mode}-state.json
