...
```
/omr-schedule "watch PR #42 merge" --every 2m --until merged
/omr-schedule "poll deploy status" --every 5m --until "status == healthy"
/omr-schedule "check CI pipeline" --every 3m
/omr-schedule list                    # show all scheduled tasks
/omr-schedule cancel <task-id>        # cancel a task
/omr-schedule resume                  # resume tasks from previous session
/omr-schedule "watch agents-radar RSS" --every 15m --scope user
```
...
- project scope: `.omr/state/schedule-state.json`
...
For the built-in RSS watcher task type, OMR also exposes a thin CLI worker:

```bash
omr schedule add-rss --scope user --url <feed> --every 15m
omr schedule worker --scope user --once
```
...
2. Subagent posts result to `.omr/state/schedule-state.json`
3. Subagent emits an OMR core notification so the user gets:
...
```bash
omr notify emit --source schedule --task-id "<task-id>" --status info --summary "<summary>" --details "<details>" --scope user
```
...
- project scope: `.omr/state/schedule-resume-pending.json`
...
**Boundary note:** Keep schedule generic inside OMR core. If a downstream repo
...
| Command | Action |
|---------|--------|
| `/omr-schedule "<task>"` | Register and start a new scheduled task |
| `/omr-schedule list` | Show all tasks (active, suspended, completed) |
| `/omr-schedule cancel <id>` | Cancel a running task |
| `/omr-schedule resume` | Resume all suspended tasks from previous session |
| `/omr-schedule status` | Show current task states with last results |
