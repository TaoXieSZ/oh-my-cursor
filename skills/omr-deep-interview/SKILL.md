...
Write the brief to `.omr/plans/interview-brief-{slug}.md` for downstream skills to consume. Do NOT start implementation. Suggest the next workflow stage:
...
`$deep-interview` persists state to `deep-interview-state.json`. **Automatic Slack webhooks are not wired for this mode** — only **forge** state writes (via the OMR state API / MCP) trigger optional Incoming Webhook posts. Use `omr notify slack` to test your webhook, or extend OMR in a future change if you need interview milestones in Slack.

## State

Write interview progress to `.omr/state/deep-interview-state.json`:
```json
{
